...
static int demo_post_file(CelHttpWebClient *api_client);
                     ...
                     const TCHAR *address, CelSslContext *api_sslctx)
{
    ...
    cel_httproute_add(route, CEL_HTTPM_POST, "post/file", 
        (CelHttpHandleFunc)demo_post_file);
...
int demo_conf_get(CelHttpWebClient *api_client)
{
    ...
    cel_httpresponse_send_file(rsp, "/etc/sunrun/cel-demo.conf", 0, 0, 
        (CelDateTime *)cel_httprequest_get_header(
        req, CEL_HTTPHDR_IF_MODIFIED_SINCE), NULL);
...
static int demo_html(CelHttpWebClient *api_client)
{
    ...
    printf("%s %s\r\n", path, cel_httpwebclient_get_remoteaddr_str(api_client));
    ...
    cel_httpresponse_send_file(rsp, path, 0, 0, 
        (CelDateTime *)cel_httprequest_get_header(&(api_client->req), 
        CEL_HTTPHDR_IF_MODIFIED_SINCE), NULL);

    return cel_httpwebclient_async_send_response(api_client, NULL);
}

int demo_post_file(CelHttpWebClient *api_client)
{
    CelHttpRequest *req = cel_httpwebclient_get_request(api_client);
    ...
    return cel_httpwebclient_async_send_response_result(
        api_client, CEL_HTTPWEB_OK, 0, NULL);
    //char buf[128];
...
int demo_service_get_version(CelHttpWebClient *api_client)
{
    ...
    //_tprintf("xx %s\r\n", cel_httpwebclient_get_remoteaddr_str(api_client));
    //post_file();
    //_tprintf("yy %s\r\n",  cel_httpwebclient_get_remoteaddr_str(api_client));
    send_request_test();
...
int demp_orgs_users_get(CelHttpWebClient *api_client)
{
    ...
    cel_httproutedata_get_string(rt_dt, "org_name", value, 128);
    //puts(value);
    cel_httproutedata_get_string(rt_dt, "user_name", value, 128);
    cel_httpresponse_resize_send_buffer(&(api_client->rsp), 256 * 1024);
    buf = cel_httpresponse_get_send_buffer(&(api_client->rsp));
    size = cel_httpresponse_get_send_buffer_size(&(api_client->rsp));
    memset(buf, 'x', size);
    cel_httpresponse_seek_send_buffer(&(api_client->rsp), size);
    //printf("size = %d %d\r\n", size, 256 * 1024);
    //puts(value);
    return cel_httpwebclient_async_send_response_result(api_client, CEL_HTTPWEB_OK, 0, NULL);
}
